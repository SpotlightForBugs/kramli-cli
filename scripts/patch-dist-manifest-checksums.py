#!/usr/bin/env python3
"""Patch cargo-dist manifest JSON with SHA-256 values from *.sha256 sidecar files.

After macOS notarization repacks archives, sidecars are updated but dist-manifest.json
still has pre-notarize checksums. Global installers (shell, sha256.sum, Homebrew)
embed those stale values unless we patch the manifest first.
"""
...
def main() -> int:
    ...
    print(
        ...
        + (f" ({skipped} sidecar(s) skipped)" if skipped else ""),
        file=sys.stderr,
    )
    return 0
